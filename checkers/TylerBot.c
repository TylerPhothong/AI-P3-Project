...
// Additional weights for advanced evaluation
#define KING_VALUE 5.0
#define PIECE_VALUE 1.0
#define CENTER_BONUS 0.5
#define BACK_ROW_PROTECTION 1.0
...
int LowOnTime(void) 
{
    clock_t current = times(&bff);
    float total = (float)(current - start) / CLK_TCK;
    return total >= (SecPerMove - 0.5);
}
...
// Improved evaluation function for board position
        ...
        for (int x = 0; x < 8; x++) {
            if (x % 2 != y % 2) {
                if (king(currBoard->board[y][x])) {
                    score += (currBoard->board[y][x] & White) ? KING_VALUE : -KING_VALUE;
                    if (y == 0 || y == 7) score += BACK_ROW_PROTECTION; // Protecting back row
                } else if (piece(currBoard->board[y][x])) {
                    score += (currBoard->board[y][x] & White) ? PIECE_VALUE : -PIECE_VALUE;
                    if (y >= 2 && y <= 5 && x >= 2 && x <= 5) score += CENTER_BONUS; // Center control
                }
            }
        }
    }
    return me == 1 ? -score : score;
}
...
{
    if (depth <= 0) return evalBoard(prevState);

    struct State state = *prevState;
    state.player = 3 - prevState->player;
    FindLegalMoves(&state);

    for (int i = 0; i < state.numLegalMoves; i++) {
        struct State nextState = state;
        PerformMove(nextState.board, state.movelist[i], MoveLength(state.movelist[i]));
        double rval = MinVal(&nextState, alpha, beta, depth - 1);
...
double MinVal(struct State *prevState, double alpha, double beta, int depth) 
{
    ...
    for (int i = 0; i < state.numLegalMoves; i++) {
        struct State nextState = state;
        PerformMove(nextState.board, state.movelist[i], MoveLength(state.movelist[i]));
        double rval = MaxVal(&nextState, alpha, beta, depth - 1);
...
// Updated FindBestMove to avoid depth increments and simplify move selection
void FindBestMove(int player)
{
    ...
    int bestMoveIndex = -1;
    int fixedDepth = 4; // Set a fixed depth to avoid overextension

    for (int i = 0; i < state.numLegalMoves && !LowOnTime(); i++) {
        struct State nextState = state;
        PerformMove(nextState.board, state.movelist[i], MoveLength(state.movelist[i]));
        double score = MinVal(&nextState, alpha, beta, fixedDepth - 1);
